...
# Detect architecture - supports both x86_64 and aarch64/arm64
...
# Operator SDK binary location based on detected architecture
OPERATOR_SDK ?= $(LOCALBIN)/$(ARCH)/operator-sdk
...
ifeq ($(OS), Darwin)
	mkdir -p $(LOCALBIN)/$(ARCH)/
ifeq ($(ARCH), aarch64)
	curl -L https://mirror.openshift.com/pub/openshift-v4/arm64/clients/operator-sdk/4.17.0/operator-sdk-v1.36.1-ocp-darwin-aarch64.tar.gz? | tar -xz -C bin/
else
	...
	mkdir -p $(LOCALBIN)/$(ARCH)/
ifeq ($(ARCH), aarch64)
	curl -L https://mirror.openshift.com/pub/openshift-v4/arm64/clients/operator-sdk/4.17.0/operator-sdk-v1.36.1-ocp-linux-aarch64.tar.gz? | tar -xz -C bin/
else
...
# Legacy GPRC catalog format (deprecated but still supported)
...
# New FBC (File-Based Catalog) format
